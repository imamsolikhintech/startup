    ...
    <nav v-if="breadcrumbs && breadcrumbs.length > 0" class="page-header__breadcrumbs">
      <ol class="breadcrumbs">
        <li v-for="(crumb, index) in breadcrumbs" :key="index" class="breadcrumbs__item">
          <router-link v-if="crumb.to" :to="crumb.to" class="breadcrumbs__link">
            {{ crumb.label }}
          </router-link>
    ...
    <div class="page-header__content">
      <div class="page-header__text">
        <div class="page-header__title-wrapper">
          ...
          <div v-if="status" :class="`page-header__status page-header__status--${status}`">
            <span class="page-header__status-indicator"></span>
            <span class="page-header__status-text">{{ status }}</span>
          </div>
        </div>
        <p v-if="subtitle" class="page-header__subtitle">{{ subtitle }}</p>
        <div v-if="meta" class="page-header__meta">{{ meta }}</div>
      </div>
      
      <div v-if="actions && actions.length > 0" class="page-header__actions">
        <button
        ...
        >
          ...
          />
          <div v-if="action.loading" class="page-header__action-spinner"></div>
